...
# Copyright (C) 2013 Infamous Open Source Project
#
...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from IOSP Github (http://github.com/Infamous-Open-Source-Project)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/Infamous-Open-Source-Project/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/infamous_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    f = open('.repo/local_manifests/infamous_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "SlimRoms/%s" % repo_name, "revision": "kitkat-4.4" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/lean.dependencies'
    syncable_repos = []
...
print "Repository for %s not found in the IOSP Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/infamous_manifest.xml" % device